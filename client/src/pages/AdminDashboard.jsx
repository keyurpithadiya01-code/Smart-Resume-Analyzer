...
function AdminDashboardContent() {
  ...
  const [pdfModal, setPdfModal] = useState({ isOpen: false, fileURL: null, userName: '' });

  const handleViewResume = async (user) => {
    try {
      const res = await api.get(`/admin/users/${user._id}/resume`, { responseType: 'blob' });
      const fileURL = window.URL.createObjectURL(new Blob([res.data], { type: 'application/pdf' }));
      setPdfModal({ isOpen: true, fileURL, userName: user.name || user.email });
    } catch (err) {
      ...
      {pdfModal.isOpen && (
        <div style={{
          ...
          <div style={{
            ...
            background: 'var(--ink)', borderBottom: '1px solid var(--ink-border-strong)'
          }}>
            <div style={{ color: 'var(--text-primary)', fontSize: '15px', fontWeight: '500', fontFamily: 'var(--font-display)' }}>
              Resume: {pdfModal.userName}
            </div>
            <button 
              onClick={() => setPdfModal({ isOpen: false, fileURL: null, userName: '' })}
              style={{
